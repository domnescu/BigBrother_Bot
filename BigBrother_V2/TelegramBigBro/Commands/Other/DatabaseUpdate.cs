    ...
    internal class DatabaseUpdateTelegram : CommandTelegram
    {
        ...
        public override async Task Execute(Message message, ITelegramBotClient botClient, CancellationToken cancellationToken)
        {
            ...
            if (user.IsAdmin && message.ForwardFrom == null)
            {
                ...
                {
                    answer = message.Text.ToLower().StartsWith("delete") && !Succes
                        ? "Произошла ошибка при удалении элемента из базы данных - проверь правильность ввода!"
                        : message.Text.ToLower().StartsWith("update") && Succes
                                            ? "Элемент из базы данных успешно обновлён."
                                            : message.Text.ToLower().StartsWith("update") && !Succes
