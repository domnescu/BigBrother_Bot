    ...
    internal class GetListOfConversations : Command
    {
        ...
        public override void Execute(Message message, VkApi client)
        {
            ...
            foreach (long peer_id in possibleChats)
            {
                try
                {
                    ConversationResult chat = client.Messages.GetConversationsById(new[] { peer_id });
                    if (chat.Items.ElementAt(0).CanWrite.Allowed)
                    {
                        @params.Message += "\npeer_id=" + peer_id + " " + chat.Items.ElementAt(0).ChatSettings.Title + ":" + chat.Items.ElementAt(0).ChatSettings.MembersCount + " участников";
